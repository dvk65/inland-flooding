...
df = pd.read_csv('data/s2_selected.csv')

# step 2 - add the image data to df
df_mod  = kmeans_utils.add_image_data(df)

# step 3 - run default KMeans 
df_kmeans_default = kmeans_utils.kmeans_clustering_default(df_mod, default_n_clusters, 'default')

# step 4 - optimize KMeans
kmeans_utils.select_n_clusters(df_mod.iloc[0], 'default')
