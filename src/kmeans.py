...
# step 2 - add the image data to df
df_mod, result_df_ndwi  = kmeans_utils.add_image_data(df_id_flood)
...
init = 'k-means++'
results_df_default = kmeans_utils.kmeans_clustering_default(df_scaled, init, default_n_clusters)

# step 5 - optimize KMeans using different combinations
# step 5.1 - optimize KMeans by applying PCA to each image
result_df_pca_i = kmeans_utils.kmeans_optimization_individual_pca(df_scaled, init, check=True)

# step 5.2 - optimize KMeans by adding flowline as a feature and applying PCA to each image
result_df_pca_flowline = kmeans_utils.kmeans_optimization_individual_pca_features(df_scaled, init, 'pca_flowline', check=True)

# step 5.3 - optimize KMeans by adding NDWI as a feature and applying PCA to each image
result_df_pca_ndwi = kmeans_utils.kmeans_optimization_individual_pca_features(df_scaled, init, 'pca_ndwi', check=True)

# step 5.4 - optimize KMeans by adding flowline and NDWI as features and applying PCA to each image
result_df_pca_features = kmeans_utils.kmeans_optimization_individual_pca_features(df_scaled, init, 'pca_features', check=True)
